    ...
    def retrieve(
        ...
        # 다수의 Query가 포함된 Dataset이 입력된 경우
            ...
            for idx, query in enumerate(queries):
                ...
                if idx<10:
                    if torch.isnan(passage_vecs).any():
                        print("Passage embedding contains NaN values")
                        nan_indices = torch.isnan(passage_vecs).nonzero(as_tuple=True)
                        print(f"NaN indices: {nan_indices}")
                    else:
                        print("No NaN values in passage embedding")
                        # Query embedding에서도 NaN 검사
                    if torch.isnan(query_vec).any():
                        print("Query embedding contains NaN values")
                        nan_indices = torch.isnan(query_vec).nonzero(as_tuple=True)
                        print(f"NaN indices: {nan_indices}")
                    else:
                        print("No NaN values in query embedding")
