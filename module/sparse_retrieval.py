...
class Sparse_and_BM25Retrieval:
    ...
    def retrieve(
        self, query_or_dataset: Union[str, Dataset], topk: Optional[int] = 1, eval: Optional[bool] = False
    ) -> Union[Tuple[List, List], pd.DataFrame]:
        """
        Arguments:
            query_or_dataset (Union[str, Dataset]):
        ...
        if isinstance(query_or_dataset, Dataset):
            ...
            with timer("query exhaustive search"):
                doc_scores, doc_indices = self.get_relevant_doc_bulk(query_or_dataset["question"], k=topk)
            if eval==True:
                return [[self.contexts[pid] for pid in doc_indices[idx][:topk]] for idx in range(len(query_or_dataset))]
            for idx, example in enumerate(tqdm(query_or_dataset, desc="Sparse retrieval: ")):
